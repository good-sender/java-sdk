#!/usr/bin/env bash
# Single-language conformance driver for good-sender/java-sdk.
#
# Usage:
...
build_sdk() {
  # Install the SDK to the local Maven cache so the test pom can resolve it.
  if ! mvn -q dependency:get -Dartifact="com.goodsender:goodsender-sdk:0.1.0" -o >/dev/null 2>&1; then
    (cd "$SDK_DIR" && mvn -q install -DskipTests) || exit 1
  fi
}

install_runner() { :; }   # mvn resolves on demand
...
case "$MODE" in
  mock)
    ...
    build_sdk
    install_runner
    (cd "$SCRIPT_DIR" && mvn -q -DskipTests compile exec:java -Dexec.mainClass=Runner)
    ;;
  dev)
    ...
    build_sdk
    install_runner
    (cd "$SCRIPT_DIR" && mvn -q -DskipTests compile exec:java -Dexec.mainClass=Conformance)
    ;;
